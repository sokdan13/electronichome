...
fun AppNavGraph(navController: NavHostController) {
    ...
    Scaffold(
        contentWindowInsets = WindowInsets(0), bottomBar = {
        ...
        NavHost(
            ...
            composable(Screen.Login.route) {
                ...
                LaunchedEffect(state.isSuccess, state.role) {
                    if (state.isSuccess && state.role != null) {
                        val dest =
                            if (state.role == UserRole.MANAGEMENT) Screen.ManagementHome.route
                            else Screen.Home.route
            ...
            composable(Screen.Meters.route) {
                ...
                if (apartment != null) {
                    MetersScreen(
                        apartment = apartment, onNavigateBack = { navController.popBackStack() })
                }
            }
